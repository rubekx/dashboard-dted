...
class DashboardController extends Controller
{
    ...
    public function ticketQueryWhere($ticket_status)
    {
        $response = null;
        if (!empty($ticket_status)) {
            $response = "WHERE ost_thread_event.id IN (SELECT MAX(ost_thread_event.id) FROM ost_thread_event";
            switch ($ticket_status) {
                case 1:$response = "{$response} GROUP BY ost_thread_event.thread_id)";
                    break;
                case 2:$response = "{$response} GROUP BY ost_thread_event.thread_id) AND ost_ticket.status_id = 3";
                    break;
                case 3:$response = "{$response} WHERE ost_thread_event.state ='reopened' GROUP BY ost_thread_event.thread_id) AND ost_ticket.status_id IN (1,6)";
                    break;
                case 4:$response = "{$response} WHERE ost_thread_event.state ='transferred' GROUP BY ost_thread_event.thread_id) AND ost_ticket.status_id IN (1,6)";
                    break;
                case 5:$response = "{$response} WHERE ost_thread_event.state ='overdue' GROUP BY ost_thread_event.thread_id) "
                    ...
                    break;
                case 6:$response = "{$response} GROUP BY ost_thread_event.thread_id) AND ost_ticket.status_id IN (1,6)";
                    break;
            }
        }
        return $response;
